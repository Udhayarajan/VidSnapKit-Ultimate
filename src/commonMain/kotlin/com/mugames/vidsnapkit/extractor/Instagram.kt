...
class Instagram internal constructor(url: String) : Extractor(url) {
    ...
    private suspend fun getQueryHashFromAllJSInPage(page: String): MutableList<String>? {
        ...
        while (matcher.find()) {
            ids.addAll(getQueryHash(httpRequestService.getResponse(matcher.group(1), headers) ?: run {
                return null
            }))
        }
        return ids
    ...
    private suspend fun extractFromItems(items: JSONArray) {
        for (i in 0 until items.length()) {
            ...
            if (formats.title.isEmpty()) {
                ...
                formats.title = Util.filterName(caption?.getNullableString("text") ?: run {
            ...
            videoVersion?.let {
                ...
                for (j in 0 until it.length()) {
                    ...
                    format.videoData.add(
                        VideoResource(
                            video.getString("url"), MimeType.VIDEO_MP4, try {
